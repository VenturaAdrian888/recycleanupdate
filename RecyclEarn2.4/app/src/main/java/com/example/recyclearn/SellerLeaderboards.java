...
//Display user info in drawer
        ...
        reference.addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(SellerLeaderboards.this, "Something wrong happened!", Toast.LENGTH_SHORT).show();
            }
        });
...
//display profile picture
        ...
        fAuth = FirebaseAuth.getInstance();
        storageReference = FirebaseStorage.getInstance().getReference();
        user1 =(CircleImageView) findViewById(R.id.sell1);

        StorageReference prof1 = storageReference.child("user_seller/"+ fAuth.getCurrentUser().getUid()+"/profile.jpg");
        prof1.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            @Override
            public void onSuccess(Uri uri) {
                Picasso.get().load(uri).into(user1);
            }
        });
        ...
        fAuth = FirebaseAuth.getInstance();
        storageReference = FirebaseStorage.getInstance().getReference();
        user2 =(CircleImageView) findViewById(R.id.sell2);

        StorageReference prof2 = storageReference.child("user_seller/"+ fAuth.getCurrentUser().getUid()+"/profile.jpg");
        prof2.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            @Override
            public void onSuccess(Uri uri) {
                Picasso.get().load(uri).into(user2);
            }
        });
